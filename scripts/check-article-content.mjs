...
const pattern = /\n  \{\n    slug: "([^"]+)",\n    title: "((?:[^"\\]|\\.)*)",([\s\S]*?)(?=\n  \},\n  \{|\n\];)/g;
const prohibited = /必ず|確実に|絶対に|完全に安全|副作用はない|必ず治る|おすすめです|最先端|世界初|No\. ?1|唯一の治療/gi;
const issues = [];
const slugs = new Set();
for (const match of body.matchAll(pattern)) {
  ...
  if (!/医師|医療|相談|確認/.test(block)) issues.push({ slug, type: "missing_medical_context_check" });
}
...
console.log(JSON.stringify({ articles: slugs.size, issues: issues.length, issueCounts: counts, samples: issues.slice(0, 30) }, null, 2));
