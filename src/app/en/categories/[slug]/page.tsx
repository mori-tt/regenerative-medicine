...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; if (!slugs.includes(slug as (typeof slugs)[number])) notFound(); const copy = localizedCategoryFor("en", slug as (typeof slugs)[number]); return pageMetadata(copy.title, copy.description, `/en/categories/${slug}/`, false, "en"); }
