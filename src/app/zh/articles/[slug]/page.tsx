...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; const source = articles.find((article) => article.slug === slug); if (!source) notFound(); const article = localizedArticleFor("zh", source); return pageMetadata(article.title, article.description, `/zh/articles/${slug}/`, false); }
