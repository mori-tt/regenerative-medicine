...
export function LocalizedArticle({ locale, source }: { locale: SiteLocale; source?: Article }) {
  const article = source ? localizedArticleFor(locale, source) : localizedArticle[locale];
  const en = locale === "en";
  return <div lang={en ? "en" : "zh-CN"} className="localized-page"><div className="container inner-page localized-article"><div className="page-heading"><span className="eyebrow">{article.category}</span><h1>{article.title}</h1><p>{article.description}</p></div><div className="key-points"><h2>{en ? "Key points" : "文章要点"}</h2><ul>{article.points.map((point) => <li key={point}>{point}</li>)}</ul></div><div className="article-body">{article.sections.map(([title, first, second], index) => <section id={`section-${index + 1}`} key={title}><h2>{title}</h2><p>{first}</p><p>{second}</p></section>)}</div><section className="references"><h2>{en ? "Sources and further reading" : "参考资料"}</h2><p>{en ? "For current regulatory information, check the official pages of Japan’s Ministry of Health, Labour and Welfare and PMDA." : "关于最新制度和产品信息，请查阅日本厚生劳动省和 PMDA 的官方资料。"}</p><a href="https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/kenkou_iryou/iryou/saisei_iryou/index.html" target="_blank" rel="noopener noreferrer">{en ? "MHLW: Regenerative medicine" : "厚生劳动省：再生医学"} ↗</a></section><Link className="text-link" href={`/${locale}/`}>{en ? "Back to the home page" : "返回首页"}</Link></div></div>;
}
